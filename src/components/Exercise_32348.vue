<template>
    <div class="container">
        ...
        <div id="questions32348" class="text-left">
            <ol>
                <li v-for="(question, i) in questions" v-bind:key="i">
                    {{ question.question }} <br>
                    <textarea id="txt32348" v-model="question.answer"></textarea>
                </li>
            </ol>
        </div>
        <!-- MODALS -->
...
<script>
import questions32348 from '@/assets/json/exercise32348.json'
...
export default {
    ...
    methods: {
        show(){
            var max = Object.keys(questions32348).length;
            for (var i = 0; i < max; i++) {
                var item = this.randomSent(max);
                var datos = {
                    question: questions32348[item].question,
                    answer: questions32348[item].answer
                };
        ...
        download(){
            ...
            html2canvas(document.querySelector('#questions32348')).then(function (canvas) {
